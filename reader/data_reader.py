...
from base_reader import Reader, EmptyReaderException
...
class DataReader:
    ...
    def read_data(self) -> pd.DataFrame:
        ...
        data = [file_set.read_data_set(INDEX_TYPE.NUMBER, DATA_TYPE.R2) for file_set in self.file_set_container]
        print(data)
        return data

    def set_file_sets_container(self, dir_names: List[str]):
        """Установить контейнер выборок файлов"""
        self.file_set_container = [FileSet(Dir(dir_name)) for dir_name in dir_names]
